...
def treinar_knn(X_train, X_test, y_train, y_test, save_path="outputs/", n_neighbors=5):
    # Criar diretórios
    ...
    if y_pred_proba is not None:
        ...
        plt.plot(
            fpr, tpr, color="#8B008B", lw=2, label=f"ROC curve (AUC = {roc_auc:.4f})"
        )
        ...
        roc_fig_path = os.path.join(save_path, "figures", "roc_knn.png")
    ...
    # =====================================================
    joblib.dump(model, os.path.join(save_path, "models", "knn_classifier.pkl"))

    with open(
        os.path.join(save_path, "metrics", "knn_classifier_metrics.json"), "w"
    ) as f:
